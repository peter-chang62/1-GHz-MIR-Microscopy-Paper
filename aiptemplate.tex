...
% highlight the need to push to yet even higher rep-rates!
As highlighted by the void in the upper right region of \mbox{Fig. \ref{fig:bckgnd}}, hyperspectral imaging that simultaneously affords 1000 cm$^{-1}$ coverage at megahertz acquisition rates is challenging to achieve. However, this presents an opportunity for scanned imaging with DCS platforms, provided the existence of broad bandwidth mid-infrared frequency combs with 10 GHz mode spacing. Generally speaking, this is a challenging regime for frequency combs, although such mode-spacings can be realized on EOM platforms  \cite{kowligyMidinfraredFrequencyCombs2020}, and in a few cases have even been used for imaging \cite{ullahkhanDirectHyperspectralDualcomb2020,khanSubGHzOpticalResolution2023}. EOM combs spanning bandwidths greater than 1000 cm$^{-1}$ at 10 GHz have been demonstrated in the near-infrared \cite{carlsonBroadbandElectroopticDualcomb2020}, but a  challenge remains in efficient and broad bandwidth down conversion to the mid-infrared\cite{kowligyMidinfraredFrequencyCombs2020}. \textcolor{red}{On a very different platform, microcombs \cite{yuSiliconchipbasedMidinfraredDualcomb2018,yuMicrofluidicMidinfraredSpectroscopy2019} can generate coherent frequency combs spanning up to a few hundred wavenumber. However, coverage well past \mbox{$\lambda \approx 3 \; \mu m$} \mbox{(3333 $\mathrm{cm^{-1}}$)} remains an outstanding challenging on thin film}. Nonetheless, this is a promising direction to explore, and our present effort with 1 GHz mid-infrared dual comb imaging is a step towards even higher bandwidths and acquisition rates as given by Eq. \ref{eq:dcs}.

% \textcolor{red}{Lastly, the diversity of methods already applied in this field lead to the perspective that there are additional promising spectroscopic techniques that remain to be extended into mid-infrared hyperspectral imaging. The advanced rapid scan type of FT-IR \cite{hashimotoHighSpeedFourier2021} and FT-CARS \cite{hashimotoUltrabroadbandRapidscanFouriertransform2018}, for instance, are capable of drastically increasing the mechanical scan rate of the classical FT-IR interferometer. Furthermore, time-stretch spectroscopy, in particular, has the potential to hit the maximum spectral acquisition rate matching the rep-rate of the laser. Although this method requires infrared optics with well-controlled dispersion \cite{kawaiCompressiveDualcombSpectroscopy2021}, the utilization of up-conversion to the near-infrared \cite{wenBroadbandConversionMid2024,yagiMidinfraredOpticalCoherence2024} or coherent Stokes Raman scattering \cite{nakamuraBroadbandCoherentRaman2023} can extend its applicability to longer wavelengths.}

% A few important mentions include time-stretch spectroscopy, and the rapid-scan type FT-IR or FT-CARS. 
...
% \begin{figure}[h]
%     \centering
%     \includegraphics[width=\linewidth]{fig/spectrum_in_setup.png}
...
\begin{figure}[!h]
    \centering
    \includegraphics[width=\linewidth]{fig/setup_3D_v4.png}
